...
/// Per-user application data. Nothing is sent off the machine.
...
/// Start a bounded local log file and return its path.
pub fn init_logging() -> anyhow::Result<std::path::PathBuf> {
    use tracing_subscriber::util::SubscriberInitExt;

    let dir = app_dir();
    std::fs::create_dir_all(&dir)?;
    let path = dir.join("vulpicast.log");
    let previous = dir.join("vulpicast.previous.log");
    if path
        .metadata()
    ...
    let filter = tracing_subscriber::EnvFilter::new(
        std::env::var("VULPICAST_LOG").unwrap_or_else(|_| "warn,vulpicast=info".into()),
    );
    tracing_subscriber::fmt()
        ...
        .finish()
        .try_init()?;
    tracing::info!(version = env!("CARGO_PKG_VERSION"), "VulpiCast started");
    Ok(path)
}

fn volume_file() -> Option<std::path::PathBuf> {
    Some(app_dir().join("volume.txt"))
}
...
impl Session {
    ...
    pub async fn start(mut device: Device, volume: f32) -> anyhow::Result<Self> {
        ...
        tracing::info!(
            ...
            model = %device.model,
            address = %ipv4,
            "using native AirPlay 2 path (legacy RAOP disabled)"
        );
    ...
    pub async fn feedback(&mut self) -> anyhow::Result<()> {
        self.conn.send_feedback().await?;
        Ok(())
    }
